 ...
 * @LastEditors: Qbenben
 * @LastEditTime: 2020-05-19 21:08:36
...
class Tree {
    ...
    /**
     * @description 插入子节点(非递归)
     * @param {Number} data: 插入的数值
    ...
    private insertNode(node: TreeNode, newNode): TreeNode {
        if (newNode.data == node.data) {
            ...
            if (node.rightChild === null) {
                ...
                return this.insertNode(node.rightChild, newNode);
            }
        }
    }
    ...
    private removeNode(current: TreeNode, data: number): null | TreeNode {
        ...
        if (data == current.data) {
            ...
            //两个节点
                ...
                current.rightChild = this.removeNode(
                    current.rightChild,
                    temNode.data
                );
                return current;
